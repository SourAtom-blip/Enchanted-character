...
export default function AdminCharacters() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editing._id) {
        await api.put(`/api/characters/${editing._id}`, editing);
      } else {
        await api.post('/api/characters', editing);
      }
      setEditing(null);
      load();
    } catch (err) {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <h1 className="font-headline-md text-headline-md text-on-surface">Characters</h1>
        <button
          type="button"
      ...
      {editing && (
        <form onSubmit={handleSave} className="bg-surface-container rounded-xl border border-secondary/10 p-6 mb-8 space-y-4">
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="font-label-md text-label-md text-on-surface-variant block mb-1">Tag</label>
              <input
                value={editing.tag}
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item._id} className="border-b border-secondary/5">
                ...
                <td className="p-4 text-on-surface-variant">{item.tag}</td>
